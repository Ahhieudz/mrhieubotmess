...
const DELAY = 1500; // cứ mỗi 1 giây chửi 1 lần
const CAU_CHUI = [
  "cho an cut cay 🤣🤣 {name}",
  "sua de 🤣🤣 {name}",
  "cho bu cut🤣🤣👌{name}",
...
async function onCall({ message, args }) {
  ...
  if (!mentionId && !isCallingStop) {
    return message.reply("Vui lòng tag người cần chửi");
  }
  if (isCallingStop) {
    if (!global.chuilientuc.has(message.threadID)) {
      return message.reply("Bot không đang chửi ai cả");
    }
    global.chuilientuc.delete(message.threadID);
    return message.reply("🔰Kduy Tha Cho Mày Rồi Đó✅✅");
  }
