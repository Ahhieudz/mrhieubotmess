...
function loadExtra(extra, pluginName) {
    ...
    try {
        ...
        logger.custom(err.message || err, ' SAKURA - LOADING ', '\x1b[31m');
    }
    ...
    for (let i = 0; i < pluginCategories.length; i++) {
        ...
        for (const plugin of categoryFiles) {
            ...
            try {
                ...
                console.error(error);
                logger.custom(getLang('modules.loader.plugins.commands.error.failed', { fileName, error }), ' SAKURA - LOADING ', '\x1b[31m');
            }
        }
    }


    logger.custom(getLang('modules.loader.plugins.commands.loaded', { commands: global.plugins.commands.size, total }), ' SAKURA - LOADING ', '\x1b[32m');
...
async function loadCustoms() {
    ...
    for (const plugin of customsFiles) {
        ...
        try {
            ...
            // console.error(error);
            logger.custom(getLang('modules.loader.plugins.customs.error.failed', { fileName, error }), ' SAKURA - LOADING ', '\x1b[31m');
        }
    }

    logger.custom(getLang('modules.loader.plugins.customs.loaded', { customs: global.plugins.customs, total: customsFiles.length }), ' SAKURA - LOADING ', '\x1b[32m');
}
...
async function loadOnMessage() {
    ...
    for (const plugin of onMessageFiles) {
        ...
        try {
            ...
            // console.error(error);
            logger.custom(getLang('modules.loader.plugins.onMessage.error.failed', { fileName, error }), ' PARADOX - LOADING ', '\x1b[31m');
        }
    }

    logger.custom(getLang('modules.loader.plugins.onMessage.loaded', { onMessage: global.plugins.onMessage.size, total: onMessageFiles.length }), ' PARADOX - LOADING ', '\x1b[32m');
}
...
async function loadEvents() {
    ...
    for (const plugin of eventsFiles) {
        ...
        try {
            ...
            // console.error(error);
            logger.custom(getLang('modules.loader.plugins.events.error.failed', { fileName, error }), ' SAKURA - LOADING ', '\x1b[31m');
        }
    }

    logger.custom(getLang('modules.loader.plugins.events.loaded', { events: global.plugins.events.size, total: eventsFiles.length }), ' SAKURA - LOADING ', '\x1b[32m');
}
